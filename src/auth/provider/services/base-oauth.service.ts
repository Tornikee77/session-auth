...
@Injectable()
export class BaseOSAuthService {
  ...
  public async findUserByCode(code: string): Promise<TypeUserInfo> {
    ...
    if (!tokensRequest.ok) {
      throw new BadRequestException(
        `failed to fetch acces token from ${this.options.profile_url}, check client_id and client_secret.`,
      );
    }
    const tokens = await tokensRequest.json();
    if (!tokens.access_token) {
      throw new BadRequestException(
        `No access_token returned from ${this.options.profile_url}.`,
      );
    }
    ...
    if (!userRequest.ok) {
      throw new UnauthorizedException(
        `cannot fetch user profile from ${this.options.profile_url}`,
      );
    }
